...
export default {async fetch(request,env){
  ...
  const url=new URL(request.url);if(request.method!=="POST"||url.pathname!=="/analyze")return json({error:"Not found"},404,origin);
  try{
    ...
    const attempt=await takeAttempt(env,user.id);if(!attempt.ok)return json({error:attempt.reason,retryAfter:attempt.retryAfter||0,remaining:attempt.remaining},429,origin);
