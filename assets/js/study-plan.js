...
const StudyPlan = (() => {
  ...
  async function init(){load();startTimer();window.dispatchEvent(new CustomEvent("afb:study-plan",{detail:plan}));if(typeof Auth!=="undefined"&&Auth._ready){Auth._ready().then(()=>hydrate()).then(()=>window.dispatchEvent(new CustomEvent("afb:study-plan",{detail:plan}))).catch(()=>{})}return plan}
