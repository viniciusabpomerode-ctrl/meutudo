(() => {
  ...
  const activePortuguese = document.getElementById("active-portuguese");
  ...
  const quizzes = [
    {
      question: "Como você diz “Bom dia!” em alemão?",
      options: ["Gute Nacht!", "Guten Morgen!", "Bis bald!", "Danke!"],
      answer: 1
    },
    {
      question: "Qual expressão pergunta informalmente “Como você está?”",
      options: ["Wie geht es dir?", "Guten Abend!", "Und Sie heißen?", "Sehr gut!"],
      answer: 0
    },
    {
      question: "Você chegou a um restaurante às 20h. Qual cumprimento combina melhor?",
      options: ["Guten Morgen!", "Gute Nacht!", "Guten Abend!", "Bis morgen!"],
      answer: 2
    },
    {
      question: "Complete: “Mir geht es ___.”",
      options: ["Tag", "bald", "gut", "Morgen"],
      answer: 2
    },
    {
      question: "Qual despedida significa “Até breve”?",
      options: ["Bis bald!", "Hallo!", "Guten Tag!", "Danke!"],
      answer: 0
    }
  ];
  ...
  function setDock(label) {
    dock.hidden = false;
    dockLabel.textContent = label || "Reproduzindo áudio";
    dockSpeed.textContent = `${String(speed).replace(".", ",")}×`;
  ...
  function showLine(speaker, de, pt, sourceButton) {
    ...
    if (activePortuguese) activePortuguese.textContent = pt;
  ...
  function playItem(item) {
    ...
    setDock(normalized.label || normalized.de || "Explicação da aula");
    audio.play().catch(() => showToast("Clique novamente para liberar o áudio no navegador."));
  }
  ...
  document.querySelectorAll("[data-play]").forEach(button => {
    button.addEventListener("click", () => {
      playQueue([{ src: button.dataset.play, button, label: button.closest(".phrase-card")?.querySelector("h3")?.textContent || "Explicação em português" }]);
  ...
  document.querySelectorAll("[data-play-sequence]").forEach(button => {
    button.addEventListener("click", () => {
      const labels = ["Curiosidade da Alemanha", "Guten Tag", "Explicação da curiosidade"];
  ...
  document.getElementById("check-answers").addEventListener("click", () => {
    ...
    if (answered < quizzes.length) {
      result.textContent = `Você respondeu ${answered} de ${quizzes.length}. Complete as que faltam e tente novamente.`;
    } else if (score === quizzes.length) {
      result.textContent = "Sehr gut! Você acertou tudo e concluiu a Aula 1.";
      localStorage.setItem("deutschbloomA1Lesson1Complete", "true");
      setProgress(100);
    } else {
      result.textContent = `Você acertou ${score} de ${quizzes.length}. Revise as marcações e tente mais uma vez.`;
  ...
  document.getElementById("next-lesson").addEventListener("click", () => {
    stopAudio();
    history.pushState({ lesson: 2 }, "", "?aula=2");
